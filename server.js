...
// One call to Gemini. grounded=true adds Google Search; grounded=false forces JSON.
...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    let text;
    try {
      text = await callGemini({ system, user, grounded: wantGrounded });
    } catch (e1) {
      ...
      if (wantGrounded) {
        text = await callGemini({ system, user, grounded: false });
      } else {
        throw e1;
      }
    }
    res.json({ content: [{ type: "text", text }] }); // shape the frontend expects
  } catch (e) {
